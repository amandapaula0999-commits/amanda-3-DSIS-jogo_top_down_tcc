...
if (!quebrado) {
    if (place_meeting(x, y, Obj_hitbox)) {
        quebrado = true;
        mask_index = -1; // Desativa a colisão física do barril
...
if (sprite_index == Spr_barril_caindo) {
    ...
    if (image_index >= _ultimo_frame) {
        image_index = _ultimo_frame; // Trava no último frame
...
if (transicao_iniciada) {
    ...
    // Quando o barril sumir por completo (alpha <= 0), finaliza a transição
    if (alpha_caindo <= 0) {
        alpha_caindo = 0;
